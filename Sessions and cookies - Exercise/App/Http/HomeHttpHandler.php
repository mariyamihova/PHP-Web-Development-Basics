...
class HomeHttpHandler extends HttpHandlerAbstract
{
    ...
    public function all(UserServiceInterface $userService,array $getData=[])
    {
        if(!empty($getData['p'])){
            ...
            $page=filter_var($getData['p'],FILTER_SANITIZE_NUMBER_INT);
            if($page>0){
                $perPage=3;
                $start=($page-1)*$perPage;
                $this->render("users/all", $userService->viewPage($start,$perPage));
            }
                            ...
                            array $formData = [],array $getData=[])
    {
        ...
        $this->handleDeleteProcess($currentUser,$userService);//$currentUser,$userService
        }
    ...
    private function handleDeleteProcess(UserDTO $currentUser,UserServiceInterface $userService)
    {
        //$user = $this->dataBinder->bind($formData, UserDTO::class);
        if ($userService->deleteProfile($currentUser)) {
            ...
            $this->render("app/error",
                new ErrorDTO("Error deleting the profile."));
        }
    }
